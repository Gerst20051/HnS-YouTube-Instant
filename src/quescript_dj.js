...
function loadQueue(){
	dC.room = getHash() || 1;
	$.getJSON("quejson.php",{type:1,dj:dC.room,apikey:"hnsapi"},function(responseData){
		if (responseData){
...
function pushQueue(){
	...
	$.get('quejson.php',{type:2,dj:dC.room,queue:Base64.encode(json_encode(dC.queue)),timestamp:dC.timestamp,apikey:"hnsapi"});
}

function monitorQueue(){
	$.getJSON("quejson.php",{type:3,dj:dC.room,timestamp:dC.timestamp,apikey:"hnsapi"},function(responseData){
...
$(window).load(function(){
	...
	$("div#l").click(function(){
		window.location.href = "quetube_index.html";
	});
});
